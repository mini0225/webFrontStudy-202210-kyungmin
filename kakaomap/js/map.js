...
class MapService {
  ...
  load() {
    this.addSearchInputEvent(); //아래꺼(addSearchInputEvent)를 받아 오려면 항상 this 사용
  ...
  addSearchInputEvent() {
    const searchInput = document.querySelector(".search-input"); //class니까 '.' 필요, selector즉 선택자라서 . 이 붙는다...?
    searchInput.onclick = () => {
      const searchRecent = document.querySelector(".search-recent");
      searchRecent.classList.toggle("invisible-recent"); //invisible-recent를 껐다 켰다 해라~ 
     }
  }
  ...
  addAsideToggleButtonEvent() {
    ...
    toggleButton.onclick =() => {
      ...
      if(aside.classList.contains("invisible-aside")) { //"invisible-aside" 이것을 포함하고 있냐??
